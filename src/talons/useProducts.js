...
export const useProducts = () => {
	...
	const { cart, setCart, setMinicartOpen } = useContext(CartContext);
	...
	}, [fetchProducts, products.length, setLoading]);
	...
	const addToCart = (id, color, quantity, openMinicart = false) => {
		const productOnCart = cart.find((cartProduct) => {
			return (
                cartProduct.id === id &&
                cartProduct.selected_color.hex_value === color.hex_value
            );
        });
	...
	const removeFromCart = (id, color) => {
		const currentProduct = cart.find((product) => {
			return (
				...
				product.selected_color.hex_value === color.hex_value
			);
		});
